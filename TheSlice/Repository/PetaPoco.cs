...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        public T ExecuteReturnScalar<T>(string sql, params object[] args)
        {
            ...
            sql = sql.Replace("@@", "@");
            var sql1 = new Sql();
            sql1.Append("declare @@returnParamter int;");
            sql1.Append("declare @returnParamter int;");
            sql1.Append(string.Format("exec @@returnParameter = '{0}';", sql),args);
            sql1.Append("select @@returnParamter");


            var ret = ExecuteScalar<T>(sql1);
            return ret;
        }
        ...
        public DataSet ExeQuery(string sql, int result, params object[] args)
        {
            ...
            try
            {
                ...
                {
                    IDataReader r;
                    DataSet poco = new DataSet();




                    var pd = PocoData.ForType(typeof(DataSet));
                    try
                    {
                        r = cmd.ExecuteReader();
                        OnExecutedCommand(cmd);
                    ...
                    // poco.MainTable.Load(r);
                    for (int i = 0; i < result; i++)
                    {
                        string name = string.Format("dt{0}", i.ToString());
                        arr[i] = name;
                        DataTable dt1 = new DataTable(name);
                        poco.Tables.Add(dt1);
                    }

                    poco.Load(r, LoadOption.OverwriteChanges, arr);
                    //dt.Load(r);
                    //dt.TableName = dt.Rows.Count > 0 ? dt.Rows[0][0].ToString() : string.Empty;
                    //dt.Columns.RemoveAt(0);
                    //poco.Tables.Add(dt);
                    //    while (!r.IsClosed&&r.ha.NextResult())
                    //    {
                    //        dt=new DataTable();
                    ...
                    return poco;
                    //var factory = pd.GetFactory(cmd.CommandText, _sharedConnection.ConnectionString, ForceDateTimesToUtc, 0, r.FieldCount, r) as Func<IDataReader, DataTable>;
                    //using (r)
                    //{
                    //    while (true)
                    //    {
        ...
        public DataTable ExecuteQuery(string sql, params object[] args)
        {
            ...
            try
            {
                ...
                {
                    IDataReader r;
                    var pd = PocoData.ForType(typeof(DataTable));
                    try
                    {
                        r = cmd.ExecuteReader();
                        OnExecutedCommand(cmd);
        ...
        public IList<Object> QueryWithMultiResults<T1, T2, T3>(string sql, params object[] args)
        {
            //if (EnableAutoSelect)
            //    sql = AddSelectClause<T>(sql);
            //var pd = PocoData.ForType(typeof(T));
            ...
            try
            {
                using (var cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    try
                    {
                        ...
                        using (r)
                        {
                            results.Add(GetResults<T1>(cmd, r).ToList());
                            r.NextResult();
                            results.Add(GetResults<T2>(cmd, r).ToList());
                            r.NextResult();
                            results.Add(GetResults<T3>(cmd, r).ToList());
        ...
        public IList<Object> QueryWithMultiResults<T1, T2, T3,T4>(string sql, params object[] args)
        {
            //if (EnableAutoSelect)
            //    sql = AddSelectClause<T>(sql);
            //var pd = PocoData.ForType(typeof(T));
                ...
                using (var cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    try
                    {
                        ...
                        using (r)
                        {
                            ...
                            results.Add(GetResults<T3>(cmd, r).ToList());
                            r.NextResult();
                            results.Add(GetResults<T4>(cmd, r).ToList());
                        }
                    }
                    catch (Exception x)
                    {
        ...
        public IList<Object> QueryWithMultiResults<T1, T2, T3,T4,T5>(string sql, params object[] args)
        {
            //if (EnableAutoSelect)
            //    sql = AddSelectClause<T>(sql);
            //var pd = PocoData.ForType(typeof(T));
                ...
                using (var cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    try
                    {
                        ...
                        using (r)
                        {
                            ...
                            results.Add(GetResults<T4>(cmd, r).ToList());
                            r.NextResult();
                            results.Add(GetResults<T5>(cmd, r).ToList());
                        }
                    }
                    catch (Exception x)
                    {
        ...
        public IList<Object> QueryWithMultiResults<T1, T2>(string sql, params object[] args)
        {
            //if (EnableAutoSelect)
            //    sql = AddSelectClause<T>(sql);
            //var pd = PocoData.ForType(typeof(T));
        ...
        private IEnumerable<T> GetResults<T>(IDbCommand cmd, IDataReader r)
        {
            var pd = PocoData.ForType(typeof(T));
            var factory = pd.GetFactory(cmd.CommandText, _sharedConnection.ConnectionString, ForceDateTimesToUtc, 0, r.FieldCount, r) as Func<IDataReader, T>;
            //using (r)
        ...
        public DataTable ExeQuery(Sql sql)
        {
            return ExecuteQuery(sql.SQL, sql.Arguments);
        }
